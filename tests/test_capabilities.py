...
def test_emit_tests_writes_feature_and_pytest(tmp_path):
    report = Report(files=[FileReport(path="cart.py", suggested_tests=[
        SuggestedTest(title="empty cart", pytest_skeleton="def test_empty():\n    assert True",
                      scenario=GherkinScenario(name="Empty cart", steps=["Given a cart", "Then ok"]))
    ])])
