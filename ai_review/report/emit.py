...
from .schema import scenario_to_feature
    ...
    for fr in report["files"]:
        ...
        if scenarios:
            feature = f"Feature: {fr['path']}, AI-suggested coverage\n\n" + "\n\n".join(
                scenario_to_feature(s) for s in scenarios
            )
            with open(os.path.join(out_dir, f"{slug}.feature"), "w", encoding="utf-8") as fh:
                fh.write(feature + "\n")
            features += 1

        skeletons = [t["pytest_skeleton"] for t in fr["suggested_tests"]
                     if t["pytest_skeleton"].strip()]
        if skeletons:
